...
const ManageColorPrint = (props) => {
  const { visible, onOk, onCancel, action, TrNo, colorPrintSelected } = props
  ...
  useEffect(() => {
    if (action === ACTION.EDIT) {
      form.setFieldsValue({
        name: colorPrintSelected.name,
        code: colorPrintSelected.code
      })
      setimageUrl(colorPrintSelected.image)
    }
  }, [colorPrintSelected])
...
export default ManageColorPrint
